...
class SignUpUseCaseTest {
    ...
    @Test
    fun `invoke should emit Loading and Success when sign-up is successful`() = runTest {
        val email = "test@example.com"
        val password = "password123"
        val expectedResult = true

        coEvery { mockRepository.emailAuthenticationSignUp(email, password, mockContext) } returns expectedResult
        ...
        coVerify(exactly = 1) { mockRepository.emailAuthenticationSignUp(email, password, mockContext) }
    }

    @Test
    fun `invoke should emit Loading and Error when sign-up fails`() = runTest {
        val email = "test@example.com"
        val password = "password123"
        val exceptionMessage = "Sign-up failed"

        coEvery { mockRepository.emailAuthenticationSignUp(email, password, mockContext) } throws Exception(exceptionMessage)
